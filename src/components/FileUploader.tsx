  ...
  return (
    <div className="rounded border border-dashed border-line bg-white p-5">
      <label className="flex cursor-pointer flex-col items-center justify-center gap-3 rounded bg-surface px-4 py-10 text-center focus-ring">
        ...
        <span className="text-sm text-steel">PDF, PNG, JPG, JPEG o DOCX - máximo {MAX_UPLOAD_MB} MB</span>
        <input
          className="sr-only"
          type="file"
      ...
      {uploadedFile && (
        <div className="mt-4 min-w-0 rounded border border-line p-3 text-sm">
          <p className="break-words font-semibold leading-5 text-ink">{uploadedFile.originalName}</p>
