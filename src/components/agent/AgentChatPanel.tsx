...
export function AgentChatPanel({ open, onClose }: { open: boolean; onClose: () => void }) {
  ...
  const [messages, setMessages] = useState<Message[]>([
    { role: "agent", content: "Hola. ¿Qué caso quieres revisar?" },
  ]);
    ...
    <div className="fixed inset-y-0 right-0 z-40 flex w-full max-w-md flex-col border-l border-line bg-white shadow-2xl">
      <div className="flex items-center justify-between border-b border-line px-4 py-3">
        <div className="flex items-center gap-2">
          <div className="grid size-9 place-items-center rounded bg-navy text-white"><Bot className="size-4" /></div>
          <div><p className="font-semibold text-ink">Asistente de auditoria</p>{intent && <p className="text-xs text-steel">{intentLabel(intent)}</p>}</div>
        </div>
        <button className="grid size-9 place-items-center rounded border border-line" onClick={onClose} aria-label="Cerrar asistente"><X className="size-4" /></button>
      </div>
      <div className="flex-1 space-y-3 overflow-y-auto p-4">
        {messages.map((message, index) => <AgentMessage key={index} {...message} />)}
        {busy && <AgentMessage role="agent" content="Analizando datos registrados..." />}
        {!!insights.length && <div className="grid grid-cols-2 gap-2">{insights.map((insight, index) => <AgentInsightCard key={index} insight={insight} />)}</div>}
      </div>
      <div className="border-t border-line p-4">
        <AgentSuggestions suggestions={suggestions} onPick={(value) => void send(value)} />
        <form className="mt-3 flex gap-2" onSubmit={submit}>
          <input className="min-w-0 flex-1 rounded border border-line px-3 py-2 text-sm focus-ring" value={input} onChange={(event) => setInput(event.target.value)} placeholder="Pregunta por facturas, alertas, siniestros..." />
          <button className="grid size-10 place-items-center rounded bg-navy text-white disabled:opacity-50" disabled={busy} aria-label="Enviar"><Send className="size-4" /></button>
        </form>
      </div>
    </div>
  );
