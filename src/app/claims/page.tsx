    ...
    <LayoutShell>
      ...
      <section className="mb-6 rounded border border-line bg-white p-4 shadow-subtle">
        ...
        <div className="grid grid-cols-[repeat(auto-fit,minmax(170px,1fr))] gap-3">
          <Input label="Número de siniestro" placeholder="Ej: 01234567" value={draft.claimNumber} onChange={(v) => setDraft({ ...draft, claimNumber: v })} />
          ...
          <Input label="Póliza asociada" placeholder="Ej: POL-AR-2024-0001" value={draft.policyNumber} onChange={(v) => setDraft({ ...draft, policyNumber: v })} />
          <Input label="Aseguradora" value={draft.insurerName} onChange={(v) => setDraft({ ...draft, insurerName: v })} />
          <Input label="Asegurado" placeholder="Ej: Juan Pérez" value={draft.insuredName} onChange={(v) => setDraft({ ...draft, insuredName: v })} />
          <Input label="Vehículo siniestrado" value={draft.vehicle} onChange={(v) => setDraft({ ...draft, vehicle: v })} />
          ...
          <label className="text-sm md:col-span-2"><span className="mb-1 block font-medium text-steel">Daño reportado</span><textarea className="min-h-20 w-full rounded border border-line px-3 py-2 focus-ring" placeholder="Ej: Impacto frontal con daño en paragolpes y óptica" value={draft.reportedDamage ?? ""} onChange={(e) => setDraft({ ...draft, reportedDamage: e.target.value })} /></label>
      ...
      <div className="w-full overflow-x-auto rounded border border-line bg-white shadow-subtle">
        <table className="w-full min-w-full text-left text-sm">
          <thead className="bg-surface text-xs uppercase text-steel"><tr><th className="px-5 py-3">Siniestro</th><th className="px-5 py-3">Factura</th><th className="px-5 py-3">Póliza</th><th className="px-5 py-3">Asegurado</th><th className="px-5 py-3">Vehículo</th><th className="px-5 py-3">Daño reportado</th><th className="px-5 py-3">Servicios</th><th className="px-5 py-3">Talleres</th><th className="px-5 py-3 text-right">Estimado</th><th className="px-5 py-3">Estado</th><th className="px-5 py-3 text-right">Acciones</th></tr></thead>
          <tbody className="divide-y divide-line">{claims.map((claim) => <tr key={claim.id}><td className="px-5 py-3 font-semibold text-navy">{claim.claimNumber}</td><td className="px-5 py-3 text-steel">{claim.invoiceNumber || "Sin dato"}</td><td className="px-5 py-3 text-steel">{claim.policyNumber}</td><td className="px-5 py-3">{claim.insuredName}</td><td className="px-5 py-3 text-steel">{claim.vehicle}</td><td className="max-w-xs px-5 py-3 text-steel">{claim.reportedDamage || "Sin dato"}</td><td className="max-w-xs px-5 py-3 text-steel">{claim.authorizedServices.join(", ") || "Sin dato"}</td><td className="max-w-xs px-5 py-3 text-steel">{claim.authorizedWorkshopNames.join(", ") || "Sin dato"}</td><td className="px-5 py-3 text-right">{formatCurrency(claim.estimatedRepairAmount)}</td><td className="px-5 py-3 text-steel">{labelFromMap(CLAIM_STATUS_LABELS, claim.status)}</td><td className="px-5 py-3"><Actions onEdit={() => setDraft(claim)} onDelete={() => remove(claim.id)} /></td></tr>)}</tbody>
        </table>
      </div>
    </LayoutShell>
  );
