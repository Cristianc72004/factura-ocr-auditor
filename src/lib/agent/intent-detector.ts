...
export function detectIntent(message: string): AgentIntent {
  ...
  if (/ayuda|puedes hacer|que sabes|como usar|como auditar|paso a paso|explicame.*pagina|flujo completo|ejemplo|que datos faltan|datos necesito/.test(text)) return "ayuda";
  ...
  if (/regla|por que regla|clasifica|riesgo|que valida|motor|generar facturas|discrepancias?|insumos|honorarios/.test(text)) return "explicar_regla";
