...
import { getAgentDataset, getClaimByNumber, getDashboardMetrics, getInvoiceById, getInvoiceByNumber } from "./audit-agent-tools";
...
export async function retrieveContext(plan: AgentPlan, memory: ConversationMemory, currentInvoiceId?: string): Promise<AgentContext> {
  ...
  const matchedClaim = getClaimByNumber(claims, claimNumber);
